        ...
        void Update()
        {
            ...
            for (int i = 0; i < Angles.Length; i++)
            {
                ...
                if (Physics.Raycast(origin, direction, out RaycastHit hit, RayDistance, CollisionLayer) && Time.time > resumeTime && !hasCollided && kart.LocalSpeed() > 0)
                {
                    ...
                    //play particle effect if hit edge
                    if(Angles[i]>0)
                    {
                        GameObject.Find("wheel_strike_right").GetComponent<ParticleSystem>().Play();
                    }
                    else if(Angles[i]<0)
                    {
                        GameObject.Find("wheel_strike_left").GetComponent<ParticleSystem>().Play();
                    }
